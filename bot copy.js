...
const { joinVoiceChannel, getVoiceConnection, VoiceConnectionStatus, NoSubscriberBehavior, createAudioPlayer, createAudioResource } = require('@discordjs/voice');
const { token, clientId, guildId, DaniLionnId } = require('./config.json');
const fs = require('node:fs');
const path = require('node:path');
const emojiCharacters = require('./emojiCharacters.js');
// const emojiRegex = require('emoji-regex/RGI_Emoji.js');
    
const audios = ['./postable-assets/fart.mp3','./postable-assets/janky-ass-music.mp3','./postable-assets/mimimimimimi.mp3', './postable-assets/laugh.mp3']
...
process.on('unhandledRejection', error =>  {
    ...
    const data = `${fs.readFile('./ErrorLog.txt', 'utf8')}\n${s}: ${error}`
    console.log(data)
...
function setBotStatus() { // Function that gives the bot a random status based on the tables above
	...
	if (activity === 'PLAYING') {
		...
		client.user.setActivity(randomMessage, { type: ActivityType.Playing });
	}
	...
	if (activity === 'LISTENING') {
		...
		client.user.setActivity(randomMessage, { type: ActivityType.Listening });
	}

    if (activity === 'COMPETING') {

		const randomMessage = CompetingMessages[Math.floor(Math.random() * CompetingMessages.length)];
		client.user.setActivity(randomMessage, { type: ActivityType.Competing });
	}
...
function getOnlineMembers(g) {
    ...
    g.members.fetch({ withPresences: true }).then(fetchedMembers => {
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.commandName === 'random-react') {
       await interaction.deferReply();
        const msgs =  interaction.channel.messages.fetch({ limit: 2 })
...
var resource = createAudioResource(audios[Math.floor(Math.random() * audios.length)])

            

            CurrentAuudioConnection.subscribe(currentAudioPlayer);
...
console.log("playing")
    ...
    if (interaction.commandName === 'change-status') {
        ...
        if (interaction.user.id === DaniLionnId) {
            ...
            await interaction.reply({ content: 'Bot status switched!', ephemeral: true });

        } else {
            await interaction.reply({ content: 'Sorry, but this command is reserved for dani only.', ephemeral: true });
        };
    }
    ...
    if (letter === ' ') {
      ...
      if (finalMessage === " ") {
        await interaction.editReply({ content: "Couldn't emojify string!", ephemeral: true });
      } else{
    ...
    if (interaction.commandName === 'insult') {
        ...
        await  interaction.channel.send(`<@${target.id}>${Insults[Math.floor(Math.random() * Insults.length)]}`); //send a random index in the array as a mention to the channel
            ...
            await interaction.editReply({ content: "HAHAHAHAH you can't make me insult myself LMAO", ephemeral: true });
    ...
    if (interaction.commandName === 'bad-compliment') {
        ...
        if (target.id != "1032426388457787402") {
            ...
            await  interaction.channel.send(`<@${target.id}> ${COmplimens[Math.floor(Math.random() * COmplimens.length)]}`); //send a random index in the array as a mention to the channel
                ...
                await interaction.editReply({ content: "HAHAHAHAH you can't make me bad compliment myself LMAO", ephemeral: true });
    ...
    if (type === 'gif_fatguy') {
        /* await */ interaction.editReply('https://i.imgur.com/r22csKw.gifv')
    }

    if (type === 'gif_buttburn') {
        /* await */ interaction.editReply('https://i.imgur.com/o86PPkN.gifv')
    }

    if (type === 'gif_deatchat') {
        /* await */ interaction.editReply('https://i.imgur.com/SPUJ8lW.gifv')
    }

    if (type === 'gif_deleted') {
        /* await */ interaction.editReply('https://i.imgur.com/cyOxq74.gifv')
    }

    if (type === 'gif_funnycat') {
        /* await */ interaction.editReply('https://i.imgur.com/VY8KeS6.gifv')
    }

    if (type === 'gif_pee') {
        /* await */ interaction.editReply('https://i.imgur.com/gz7YaDR.gifv')
    }

    if (type === 'gif_sleepycat') {
        /* await */ interaction.editReply('https://i.imgur.com/n4fhwjP.gifv')
    }

    if (type === 'gif_cock') {
        /* await */ interaction.editReply('https://i.imgur.com/FUMmPce.gifv')
    }

    if (type === 'gif_poss') {
        /* await */ interaction.editReply('https://i.imgur.com/4xfhltw.gifv')
    }

    if (type === 'gif_boomie') {
        /* await */ interaction.editReply('https://tenor.com/view/cum-penis-cum-i-creamed-cumming-xd-gif-20404521')
    }

    if (type === 'gif_doctorpiss') {
        /* await */ interaction.editReply('https://tenor.com/view/urine-test-gif-19939272')
    }

    if (type === 'gif_info') {
        /* await */ interaction.editReply('https://tenor.com/view/my-reaction-to-that-information-gif-26013120')
    }

    if (type === 'gif_dogstare') {
        /* await */ interaction.editReply('https://tenor.com/view/kinkytwt-stan-twitter-dog-gif-22789987')
    }

...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.setDescription("returns the definition of a random word (uses a free dictionary API from https://dictionaryapi.dev/)"),
	async execute(interaction) {
         ...
         resp.on('end', async () => {
           ...
           //Gotta check if we got a valid definition responce
            ...
            const embed = new EmbedBuilder()
                .setTitle(word)
                .setColor(0xFFFFFF)
                .setImage('https://danilionn.github.io/danis-bot-website/assets/images/danibot-dictionary-v2.png')
                .addFields(
                {name: partOfSpeech, value: definition}
                )
                ...
                if (partOfSpeech2 != undefined && definition2 != undefined ) {
                    embed.addFields(
                    {name: partOfSpeech2, value: definition2}
                    )}
            ...
            await interaction.editReply(`Couldn't define "${word}"!`); console.log(" "); console.log(`Error: couldn't define "${word}"!`)
        }
    });
            ...
            const read = fs.readFileSync(`${dirname}/ErrorLog.txt`, `utf-8`, errr => {if (errr) {console.log(err)}}) 

            const data =  `${read}\n${s} Command Error: ${err}`

            console.log("Error: " + err.message)

            fs.writeFileSync(`${dirname}/ErrorLog.txt`, data, errrr => {if (errrr) {console.log(errrr)}})

           await interaction.editReply("An error occurred whilst trying to execute this command!")});
       ...
       {name: "noun", value: `An odd way to spell "Conservative hellscape".`},
       {name: "Example:", value: "Where are you going? Heaven, or Ǫ̵̧̢̛̺̻̖̜̹͎͎̟͔͒̽̓̊͑̀́̃̓̂͂̕͜ͅ ̷̱͍̰̘̘̠͐̌̈̃͊̒̊͆̆͆̒͐̏̂̈́ H̵̢̛͙̟̻̰̦̜̳͉̣̑̊̓̇͗͒̎̿̎͝ ̵̛͉̭̳̼͍̖̦̓̈̏̄̍̎̇̈̾͂̕͠͝͝ I̵̩̮͆̆̈́̓ ̶̧̞̬͉̮́̾̓̃͝ O̷̢͎̘͋̏̊͘"},

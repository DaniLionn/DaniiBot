...
,
	...
	async execute(interaction) {
		...
		await interaction.channel.messages.fetch({ limit: interaction.options.get('number').value }).then(messages => {
			interaction.channel.bulkDelete(messages)
		  })

		await interaction.reply(`Purged ${interaction.options.get('number').value} messages.`)
		  setTimeout(async () => {
